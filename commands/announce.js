  ...
  run: async (client, message, args) => {
    ...
    if (!sayChannel)
      return message.channel.send(
        `<a:bearHyperYay:831216264750956565> | ${message.author} mention a channel First`
      );
    let sayMsg = args
      .slice(1)
      .join(" ")
    ...
    if (!sayMsg)
      return message.channel.send(
        `<a:bearHyperYay:831216264750956565> | Say Some Message To Announce`
      );
    ...
    message.channel
      .send(
        `<a:bearHyperYay:831216264750956565> | successfully Announced Your Message To ${sayChannel}`
      )
